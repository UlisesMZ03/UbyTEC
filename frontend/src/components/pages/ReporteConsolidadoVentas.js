...
import './ReporteConsolidadoVentas.css';

const ReporteConsolidadoVentas = () => {
  ...
  useEffect(() => {
    const fetchReporte = async () => {
      try {
        const response = await fetch('https://sqlapi-hshshrdbaba8gbgd.canadacentral-01.azurewebsites.net/api/reportes/reporte-consolidado-ventas');
        if (!response.ok) {
  ...
  const exportarPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text('Reporte Consolidado de Ventas', 105, 20, { align: 'center' });

    // Construcción de los datos para la tabla
    const tableColumn = ['Cliente', 'Afiliado', 'Conductor', 'Compras', 'Monto Total', 'Monto Servicio'];
    ...
    // Descargar el PDF
    doc.save('Reporte_Consolidado_Ventas.pdf');
  };
  ...
  return (
    <div className="reporte-consolidado-container">
      ...
      <table className="reporte-tabla">
        ...
        <tbody>
          ...
          <tr className="totales">
            <td colSpan="3" style={{ fontWeight: 'bold', textAlign: 'right' }}>Totales:</td>
            <td style={{ fontWeight: 'bold' }}>{totalCompras}</td>
...
export default ReporteConsolidadoVentas;
